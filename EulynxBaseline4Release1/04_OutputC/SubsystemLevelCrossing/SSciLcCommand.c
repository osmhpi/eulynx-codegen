...
void transition_from_SSciLcCommand__root__SendingCommands(SSciLcCommand *self, SSciLcCommand__root__state_struct *x)
{
    ...
    if (self->Change1062.IsTriggered)
    {
...
{
    self->Change867.IsTriggered = self->T1inCdActivation.IsTriggered;
    self->Change866.IsTriggered = self->T1inCdActivation.IsTriggered;
    self->Change1062.IsTriggered = self->T2inCdDeactivation.IsTriggered;
