                                                                    ...
                                                                    SSciEfesPrim__root__state_struct *x)
{
    ...
    if (self->Time24.IsTimeoutExpired)
    {
...
{
    self->Change1175.IsTriggered = self->T47inConOtherPdiVersionAvailable.IsTriggered;
    self->Change1178.IsTriggered = self->T48inDisableOrDisconnectPdiEfes.IsTriggered;
    ...
    self->Change980.IsTriggered = self->T20inProtocolError.IsTriggered;
    ...
    self->Change1179.IsTriggered = self->T48inDisableOrDisconnectPdiEfes.IsTriggered;
    self->Change742.IsTriggered = self->T10inScpConnectionTerminated.IsTriggered;
    self->Change1177.IsTriggered = self->T48inDisableOrDisconnectPdiEfes.IsTriggered;
}
...
static void resetTriggers(SSciEfesPrim *self)
{
    ...
    self->Time24.IsTimeoutExpired = false;
}
