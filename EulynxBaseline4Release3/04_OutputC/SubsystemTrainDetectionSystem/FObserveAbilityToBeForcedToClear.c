...
void make_state_FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear(
    FObserveAbilityToBeForcedToClear *self,
    FObserveAbilityToBeForcedToClear__root__state_struct *region) {
  LOG("[FObserveAbilityToBeForcedToClear] Entering state "
...
void make_state_FObserveAbilityToBeForcedToClear__root(
    FObserveAbilityToBeForcedToClear *self,
  ...
  make_state_FObserveAbilityToBeForcedToClear__root__WaitingForFinishBooting(
      self, x);
  return;
}
...
void transition_from_FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear(
    FObserveAbilityToBeForcedToClear *self,
    FObserveAbilityToBeForcedToClear__root__state_struct *x) {
  transition_from_FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear__root(
      self, x);
  if (self->Change234.IsTriggered) {
    self->D13outAbleToBeForcedToClear.Value = false;
    make_state_FObserveAbilityToBeForcedToClear__root__TechnicalDisturbance(
        self, x);
    return;
  }
...
void transition_from_FObserveAbilityToBeForcedToClear__root__WaitingForFinishBooting(
  ...
  if (self->Change403.IsTriggered) {
    ...
    if (self->D32inCriticalFailureTvps.Value) {
      ...
      make_state_FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear(
          self, x);
      return;
    }
  }
...
void transition_from_FObserveAbilityToBeForcedToClear__root(
    FObserveAbilityToBeForcedToClear *self,
    FObserveAbilityToBeForcedToClear__root__state_struct *x) {
  switch (x->state) {
  case FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear:
    transition_from_FObserveAbilityToBeForcedToClear__root__ObserveAbilityToBeForcedToClear(
        self, x);
    break;
  case FObserveAbilityToBeForcedToClear__root__WaitingForFinishBooting:
    transition_from_FObserveAbilityToBeForcedToClear__root__WaitingForFinishBooting(
        self, x);
    break;
  case FObserveAbilityToBeForcedToClear__root__FallbackMode:
    transition_from_FObserveAbilityToBeForcedToClear__root__FallbackMode(self,
                                                                         x);
    break;
  case FObserveAbilityToBeForcedToClear__root__TechnicalDisturbance:
    transition_from_FObserveAbilityToBeForcedToClear__root__TechnicalDisturbance(
        self, x);
    break;
  }
}
...
static void evaluateChangeEvents(FObserveAbilityToBeForcedToClear *self) {
  self->Change118.IsTriggered = IsTriggered(MakeChange(
      self->D14nMonitoringTime.IsSignalled, self->D14nMonitoringTime.Value));
  self->Change128.IsTriggered =
      IsTriggered(MakeChange(self->D18inPerformFcPOrFcPA.IsSignalled,
                             self->D18inPerformFcPOrFcPA.Value));
  ...
  self->Change505.IsTriggered = IsTriggered(MakeChange(
      ...
      self->D9inOccupancyStatus.Value ==
          FObserveAbilityToBeForcedToClear_D9inOccupancyStatusValue__UnreliableOut));
  self->Change549.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nMonitoringTime.IsSignalled, self->D14nMonitoringTime.Value)));
  self->Change547.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nMonitoringTime.IsSignalled, self->D14nMonitoringTime.Value)));
  self->Change548.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nMonitoringTime.IsSignalled, self->D14nMonitoringTime.Value)));
  self->Change552.IsTriggered = IsTriggered(
      NegateChange(MakeChange(self->D18inPerformFcPOrFcPA.IsSignalled,
                              self->D18inPerformFcPOrFcPA.Value)));
...
void new_FObserveAbilityToBeForcedToClear(
    FObserveAbilityToBeForcedToClear *self) {
