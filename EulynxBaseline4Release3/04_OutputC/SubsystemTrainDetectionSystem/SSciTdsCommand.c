...
void make_state_SSciTdsCommand__root__SendingCommands(
    SSciTdsCommand *self, SSciTdsCommand__root__state_struct *region) {
  LOG("[SSciTdsCommand] Entering state SendingCommands in region root");
  region->state = SSciTdsCommand__root__SendingCommands;
};

void make_state_SSciTdsCommand__root(SSciTdsCommand *self,
...
void transition_from_SSciTdsCommand__root__SendingCommands(
    SSciTdsCommand *self, SSciTdsCommand__root__state_struct *x) {

  if (self->Change1253.IsTriggered) {
    if ((self->D70inModeOfFc.Value == SSciTdsCommand_D70inModeOfFcValue__FcU) &&
        (self->D50PdiConnectionState.Value ==
  ...
  if (self->Change1249.IsTriggered) {
    if ((self->D70inModeOfFc.Value ==
      ...
      self->OutCdFc__ec57.Value.CommandedModeOfFC =
  ...
  if (self->Change1254.IsTriggered) {
    if (self->D50PdiConnectionState.Value ==
        SSciTdsCommand_D50PdiConnectionStateValue__Established) {
...
void transition_from_SSciTdsCommand__root(
    SSciTdsCommand *self, SSciTdsCommand__root__state_struct *x) {
  switch (x->state) {
