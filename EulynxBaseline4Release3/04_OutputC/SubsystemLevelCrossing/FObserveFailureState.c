...
static void evaluateChangeEvents(FObserveFailureState *self)
{
    ...
    self->Change350.IsTriggered = IsTriggered(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FObserveFailureState_D51inEstEfesStateValue__FallbackMode));
    self->Change332.IsTriggered =
        IsTriggered(MakeChange(self->D51inEstEfesState.IsSignalled,
                               self->D51inEstEfesState.Value == FObserveFailureState_D51inEstEfesStateValue__Booting));
}
