...
void make_state_FSciLcReceive__root__NationalSpecificState(
    FSciLcReceive *self, FSciLcReceive__root__state_struct *region) {
...
void transition_from_FSciLcReceive__root__NationalSpecificState(
    FSciLcReceive *self, FSciLcReceive__root__state_struct *x) {
...
static void evaluateChangeEvents(FSciLcReceive *self) {
  self->Change411.IsTriggered = IsTriggered(
      MakeChange(self->D53inTimeExpired.IsSignalled,
                 self->D53inTimeExpired.Value ==
  ...
  self->Change322.IsTriggered = IsTriggered(AndChange(
      ...
      MakeChange(self->D55inConInitalActivateLcpf.IsSignalled,
                 self->D55inConInitalActivateLcpf.Value == true)));
  self->Change323.IsTriggered = IsTriggered(AndChange(
      ...
      MakeChange(self->D55inConInitalActivateLcpf.IsSignalled,
                 self->D55inConInitalActivateLcpf.Value == true)));
  self->Change320.IsTriggered = IsTriggered(
      MakeChange(self->D51inEstEfesState.IsSignalled,
                 self->D51inEstEfesState.Value ==
