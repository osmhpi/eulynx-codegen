...
void make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Activated(
    FSciLcReceive *self,
    FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__state_struct *region)
{
...
void make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState(
    FSciLcReceive *self, FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__state_struct *x)
{
    ...
    {
        self->D1outReceiveLcState.Value = FSciLcReceive_D1outReceiveLcStateValue__Activated;
        self->D1outReceiveLcState.IsSignalled = true;
        self->T31outActivateLcpf.Trigger = true;
        make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Activated(self, x);
        return;
    }
    if (self->D55inConInitalActivateLcpf.Value == false)
    {
        self->T32outDeactivateLcpf.Trigger = true;
        self->D1outReceiveLcState.Value = FSciLcReceive_D1outReceiveLcStateValue__Deactivated;
        self->D1outReceiveLcState.IsSignalled = true;
        make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Deactivated(self, x);
        return;
    }
}
...
void make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover(
    FSciLcReceive *self,
    FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover__state_struct *x)
{
    memset(x, 0,
           sizeof(FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover__state_struct));

    make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover__Waiting(self, x);
    return;
}
                                                                    ...
                                                                    FSciLcReceive__root__state_struct *region)
{
    ...
    make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover(
        self, &region->ReceivingLevelCrossingCommands.ReceiveLocalOperationHandover);
};

void make_state_FSciLcReceive__root__NationalSpecificState(FSciLcReceive *self,
                                                           FSciLcReceive__root__state_struct *region)
{
    ...
    FSciLcReceive *self, FSciLcReceive__root__state_struct *x)
{
    ...
    if (self->InCdActivation__3513.HasMessage)
    {
        ...
        {
            self->D1outReceiveLcState.Value = FSciLcReceive_D1outReceiveLcStateValue__PreActivated;
            self->D1outReceiveLcState.IsSignalled = true;
    ...
    if (self->InCdIsolateLc__add9.HasMessage)
    {
        ...
        {
            self->D1outReceiveLcState.Value = FSciLcReceive_D1outReceiveLcStateValue__Isolated;
            self->D1outReceiveLcState.IsSignalled = true;
    ...
    FSciLcReceive *self, FSciLcReceive__root__state_struct *x)
{
    ...
    case FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Activated:
        transition_from_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Activated(self,
                                                                                                                 x);
        break;
    case FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Deactivated:
        transition_from_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Deactivated(self,
                                                                                                                   x);
        break;
    case FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Isolated:
        transition_from_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceivingActivationState__Isolated(self,
                                                                                                                x);
        break;
    ...
    if (self->InCdLocalOperationHandover__2b72.HasMessage)
    {
            ...
            LocalOperationHandoverControlableState__9164__HandoverToLocalOperatorEstablished)
        {
            ...
            make_state_FSciLcReceive__root__ReceivingLevelCrossingCommands__ReceiveLocalOperationHandover__HandoverToLocalOperatorEstablished(
                self, &x->ReceivingLevelCrossingCommands.ReceiveLocalOperationHandover);
            return;
        }
    }
    if (self->InCdLocalOperationHandover__2b72.HasMessage)
...
void transition_from_FSciLcReceive__root__ReceivingLevelCrossingCommands(FSciLcReceive *self,
                                                                         FSciLcReceive__root__state_struct *x)
{
    ...
    if (self->Change355.IsTriggered)
    {
        ...
        make_state_FSciLcReceive__root__NationalSpecificState(self, x);
        return;
    }
}

void transition_from_FSciLcReceive__root__NationalSpecificState(FSciLcReceive *self,
                                                                FSciLcReceive__root__state_struct *x)
{
...
{
    self->Change411.IsTriggered =
        IsTriggered(MakeChange(self->D53inTimeExpired.IsSignalled,
                               self->D53inTimeExpired.Value == FSciLcReceive_D53inTimeExpiredValue__DelayExpired));
    self->Change322.IsTriggered = IsTriggered(AndChange(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Initialising),
        MakeChange(self->D55inConInitalActivateLcpf.IsSignalled, self->D55inConInitalActivateLcpf.Value == true)));
    self->Change323.IsTriggered = IsTriggered(AndChange(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Initialising),
        MakeChange(self->D55inConInitalActivateLcpf.IsSignalled, self->D55inConInitalActivateLcpf.Value == true)));
    self->Change320.IsTriggered =
        IsTriggered(MakeChange(self->D51inEstEfesState.IsSignalled,
                               self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Booting));
    self->Change355.IsTriggered = IsTriggered(OrChange(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__FallbackMode),
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__NoOperatingVoltage)));
    self->Change325.IsTriggered = IsTriggered(OrChange(
        OrChange(MakeChange(self->D51inEstEfesState.IsSignalled,
                            self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Operational),
                 MakeChange(self->D51inEstEfesState.IsSignalled,
                            self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Booting)),
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Initialising)));
    self->Change319.IsTriggered =
        IsTriggered(MakeChange(self->D51inEstEfesState.IsSignalled,
                               self->D51inEstEfesState.Value == FSciLcReceive_D51inEstEfesStateValue__Booting));
}
