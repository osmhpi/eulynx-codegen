...
void make_state_FSciPReceive__root(FSciPReceive *self, FSciPReceive__root__state_struct *x)
{
    ...
    self->D10outRequiredPointPosition.Value = FSciPReceive_D10outRequiredPointPositionValue__Uncommanded;
...
void transition_from_FSciPReceive__root__ReceivingCommands(FSciPReceive *self, FSciPReceive__root__state_struct *x)
{
    ...
    if (self->InCdMovePoint__342e.HasMessage)
    {
        ...
        {
            self->D10outRequiredPointPosition.Value = FSciPReceive_D10outRequiredPointPositionValue__Left;
            self->D10outRequiredPointPosition.IsSignalled = true;
            make_state_FSciPReceive__root__ReceivingCommands(self, x);
            return;
        }
    ...
    if (self->InCdMovePoint__342e.HasMessage)
    {
        if ((self->InCdMovePoint__342e.Value.CommandedPointPositionState ==
            ...
            (self->D50inPdiConnectionState.Value == FSciPReceive_D50inPdiConnectionStateValue__Established))
        {
            self->D10outRequiredPointPosition.Value = FSciPReceive_D10outRequiredPointPositionValue__Right;
            self->D10outRequiredPointPosition.IsSignalled = true;
            make_state_FSciPReceive__root__ReceivingCommands(self, x);
            return;
        }
...
{
    self->Change39.IsTriggered = IsTriggered(
        MakeChange(self->D11inObservedAbilityToMove.IsSignalled,
                   self->D11inObservedAbilityToMove.Value == FSciPReceive_D11inObservedAbilityToMoveValue__AbleToMove));
    self->Change83.IsTriggered = IsTriggered(
        MakeChange(self->D12inRequiredPmPosition.IsSignalled,
                   self->D12inRequiredPmPosition.Value == FSciPReceive_D12inRequiredPmPositionValue__Uncommanded));
    self->Change306.IsTriggered = IsTriggered(OrChange(
        ...
        MakeChange(self->D50inPdiConnectionState.IsSignalled,
                   self->D50inPdiConnectionState.Value == FSciPReceive_D50inPdiConnectionStateValue__Suspended)));
}
