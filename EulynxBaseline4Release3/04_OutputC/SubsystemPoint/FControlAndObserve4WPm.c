...
void make_state_FControlAndObserve4WPm__root__Operating__root(
    FControlAndObserve4WPm *self, FControlAndObserve4WPm__root__Operating__root__state_struct *x)
{
    ...
    FControlAndObserve4WPm *self, FControlAndObserve4WPm__root__state_struct *x)
{
    ...
    if (self->Change71.IsTriggered)
    {
        if ((self->D20inConDriveCapability.Value) && (self->D45inConActive.Value))
        {
            ...
            make_state_FControlAndObserve4WPm__root__Operating__root__MovingLeftPm(self, &x->Operating.root);
            return;
        }
    }
    if (self->Change77.IsTriggered)
    {
        if ((self->D20inConDriveCapability.Value) && (self->D45inConActive.Value))
        {
            ...
            self->D10outPmPosition.Value = FControlAndObserve4WPm_D10outPmPositionValue__NoEndPosition;
            self->D10outPmPosition.IsSignalled = true;
            make_state_FControlAndObserve4WPm__root__Operating__root__MovingRightPm(self, &x->Operating.root);
            return;
        }
    }
    if (self->Change177.IsTriggered)
    {
        ...
        self->D10outPmPosition.IsSignalled = true;
        make_state_FControlAndObserve4WPm__root__Operating__root__Detection(self, &x->Operating.root);
        return;
    }
    ...
    {
        self->D10outPmPosition.Value = FControlAndObserve4WPm_D10outPmPositionValue__UnintendedPosition;
        self->D10outPmPosition.IsSignalled = true;
        make_state_FControlAndObserve4WPm__root__Operating__root__Detection(self, &x->Operating.root);
        return;
    }
...
void transition_from_FControlAndObserve4WPm__root__Operating__root(FControlAndObserve4WPm *self,
                                                                   FControlAndObserve4WPm__root__state_struct *x)
{
...
void transition_from_FControlAndObserve4WPm__root__WaitingForInitialising(FControlAndObserve4WPm *self,
                                                                          FControlAndObserve4WPm__root__state_struct *x)
{
...
{
    self->Change71.IsTriggered = IsTriggered(
        MakeChange(self->D12inRequiredPmPosition.IsSignalled,
                   self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Left));
    self->Change77.IsTriggered = IsTriggered(
        MakeChange(self->D12inRequiredPmPosition.IsSignalled,
                   self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Right));
    self->Change177.IsTriggered = IsTriggered(
        MakeChange(self->D27in4WPmPosition.IsSignalled,
                   self->D27in4WPmPosition.Value == FControlAndObserve4WPm_D27in4WPmPositionValue__LeftDetected));
    self->Change178.IsTriggered = IsTriggered(
        MakeChange(self->D27in4WPmPosition.IsSignalled,
                   self->D27in4WPmPosition.Value == FControlAndObserve4WPm_D27in4WPmPositionValue__NoEndPosition));
    self->Change179.IsTriggered = IsTriggered(
        MakeChange(self->D27in4WPmPosition.IsSignalled,
                   self->D27in4WPmPosition.Value == FControlAndObserve4WPm_D27in4WPmPositionValue__RightDetected));
    self->Change180.IsTriggered = IsTriggered(
        MakeChange(self->D27in4WPmPosition.IsSignalled,
                   self->D27in4WPmPosition.Value == FControlAndObserve4WPm_D27in4WPmPositionValue__UnintendedPosition));
    self->Change68.IsTriggered = IsTriggered(
        MakeChange(self->D12inRequiredPmPosition.IsSignalled,
                   self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Left));
    self->Change80.IsTriggered = IsTriggered(MakeChange(
        self->D12inRequiredPmPosition.IsSignalled,
        self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__RightReached));
    self->Change84.IsTriggered = IsTriggered(MakeChange(
        self->D12inRequiredPmPosition.IsSignalled,
        self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Uncommanded));
    self->Change74.IsTriggered = IsTriggered(MakeChange(
        self->D12inRequiredPmPosition.IsSignalled,
        self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__LeftReached));
    self->Change76.IsTriggered = IsTriggered(
        MakeChange(self->D12inRequiredPmPosition.IsSignalled,
                   self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Right));
    self->Change81.IsTriggered = IsTriggered(MakeChange(
        self->D12inRequiredPmPosition.IsSignalled,
        self->D12inRequiredPmPosition.Value == FControlAndObserve4WPm_D12inRequiredPmPositionValue__Uncommanded));
    self->Change328.IsTriggered = IsTriggered(
    ...
    self->Change372.IsTriggered = IsTriggered(OrChange(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FControlAndObserve4WPm_D51inEstEfesStateValue__NoOperatingVoltage),
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FControlAndObserve4WPm_D51inEstEfesStateValue__FallbackMode)));
    self->Change176.IsTriggered = IsTriggered(MakeChange(self->D26inInternalTriggerAbilityToMovePmAvailable.IsSignalled,
                                                         self->D26inInternalTriggerAbilityToMovePmAvailable.Value));
    self->Change557.IsTriggered =
        IsTriggered(NegateChange(MakeChange(self->D26inInternalTriggerAbilityToMovePmAvailable.IsSignalled,
                                            self->D26inInternalTriggerAbilityToMovePmAvailable.Value)));
    self->Change329.IsTriggered = IsTriggered(
        MakeChange(self->D51inEstEfesState.IsSignalled,
                   self->D51inEstEfesState.Value == FControlAndObserve4WPm_D51inEstEfesStateValue__Booting));
}
