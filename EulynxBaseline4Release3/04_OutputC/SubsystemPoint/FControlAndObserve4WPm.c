...
static void evaluateChangeEvents(FControlAndObserve4WPm *self) {
  self->Change71.IsTriggered = IsTriggered(MakeChange(
  ...
  self->Change177.IsTriggered = IsTriggered(MakeChange(
  ...
  self->Change178.IsTriggered = IsTriggered(MakeChange(
      self->D27in4WPmPosition.IsSignalled,
      self->D27in4WPmPosition.Value ==
          FControlAndObserve4WPm_D27in4WPmPositionValue__NoEndPosition));
  self->Change179.IsTriggered = IsTriggered(MakeChange(
      self->D27in4WPmPosition.IsSignalled,
      self->D27in4WPmPosition.Value ==
          FControlAndObserve4WPm_D27in4WPmPositionValue__RightDetected));
  self->Change180.IsTriggered = IsTriggered(MakeChange(
      self->D27in4WPmPosition.IsSignalled,
      self->D27in4WPmPosition.Value ==
  ...
  self->Change176.IsTriggered = IsTriggered(
      MakeChange(self->D26inInternalTriggerAbilityToMovePmAvailable.IsSignalled,
                 self->D26inInternalTriggerAbilityToMovePmAvailable.Value));
  self->Change557.IsTriggered = IsTriggered(NegateChange(
      MakeChange(self->D26inInternalTriggerAbilityToMovePmAvailable.IsSignalled,
                 self->D26inInternalTriggerAbilityToMovePmAvailable.Value)));
