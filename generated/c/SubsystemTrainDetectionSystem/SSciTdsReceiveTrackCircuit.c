...
void make_state_SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus(
    SSciTdsReceiveTrackCircuit *self,
    SSciTdsReceiveTrackCircuit__root__state_struct *region) {
...
void make_state_SSciTdsReceiveTrackCircuit__root(
    SSciTdsReceiveTrackCircuit *self,
    SSciTdsReceiveTrackCircuit__root__state_struct *x) {
  memset(x, 0, sizeof(SSciTdsReceiveTrackCircuit__root__state_struct));

  make_state_SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus(self,
                                                                         x);
  return;
}
...
void transition_from_SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus(
  ...
  if (self->InMsgTvpsOccupancyStatus__b4e1.HasMessage) {
    if ((self->InMsgTvpsOccupancyStatus__b4e1.Value.ReportedOccupancyStatus ==
         OccupancyStatus__65fa__Vacant) &&
        (self->InMsgTvpsOccupancyStatus__b4e1.Value.ReportedpomStatus ==
         PomStatus__d74a__Ok)) {
      ...
      make_state_SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus(
          self, x);
      return;
    }
  }
  ...
  if (self->InMsgTvpsOccupancyStatus__b4e1.HasMessage) {
    if ((self->InMsgTvpsOccupancyStatus__b4e1.Value.ReportedOccupancyStatus ==
      ...
      self->D66outTvpsOccupancyStatus.IsSignalled = true;
      self->D66outPomStatus.Value =
          SSciTdsReceiveTrackCircuit_D66outPomStatusValue__NotApplicable;
      self->D66outPomStatus.IsSignalled = true;
      self->D66outDisturbanceStatus.Value =
          SSciTdsReceiveTrackCircuit_D66outDisturbanceStatusValue__NotApplicable;
      self->D66outDisturbanceStatus.IsSignalled = true;
...
void transition_from_SSciTdsReceiveTrackCircuit__root(
    SSciTdsReceiveTrackCircuit *self,
    SSciTdsReceiveTrackCircuit__root__state_struct *x) {
  switch (x->state) {
  case SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus:
    transition_from_SSciTdsReceiveTrackCircuit__root__RepoprtingOccupancyStatus(
        self, x);
    break;
  }
}
