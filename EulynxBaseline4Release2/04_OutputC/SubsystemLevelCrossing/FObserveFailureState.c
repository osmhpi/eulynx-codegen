...
static void evaluateChangeEvents(FObserveFailureState *self) {
  self->Change585.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D60inLcFailure.IsSignalled, self->D60inLcFailure.Value)));
  self->Change437.IsTriggered = IsTriggered(
      MakeChange(self->D60inLcFailure.IsSignalled, self->D60inLcFailure.Value));
  self->Change586.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D60inLcFailure.IsSignalled, self->D60inLcFailure.Value)));
  self->Change439.IsTriggered = IsTriggered(
      MakeChange(self->D60inLcFailure.IsSignalled, self->D60inLcFailure.Value));
  ...
  self->Change455.IsTriggered = IsTriggered(MakeChange(
      ...
      self->D74inLcpfFailureState.Value ==
          FObserveFailureState_D74inLcpfFailureStateValue__FailurePresent));
  self->Change420.IsTriggered = IsTriggered(MakeChange(
