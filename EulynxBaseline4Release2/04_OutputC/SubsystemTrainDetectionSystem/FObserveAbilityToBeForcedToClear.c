...
static void evaluateChangeEvents(FObserveAbilityToBeForcedToClear *self) {
  self->Change110.IsTriggered = IsTriggered(MakeChange(
  ...
  self->Change512.IsTriggered = IsTriggered(MakeChange(
      ...
      self->D9inOccupancyStatus.Value ==
          FObserveAbilityToBeForcedToClear_D9inOccupancyStatusValue__UnreliableOut));
  self->Change554.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nRunningTimer.IsSignalled, self->D14nRunningTimer.Value)));
  self->Change551.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nRunningTimer.IsSignalled, self->D14nRunningTimer.Value)));
  self->Change553.IsTriggered = IsTriggered(NegateChange(MakeChange(
      self->D14nRunningTimer.IsSignalled, self->D14nRunningTimer.Value)));
