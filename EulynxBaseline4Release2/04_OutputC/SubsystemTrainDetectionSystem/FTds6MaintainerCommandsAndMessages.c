...
void make_state_FTds6MaintainerCommandsAndMessages__root__ReceivingCommandsAndReportMessages(
    ...
    FTds6MaintainerCommandsAndMessages__root__state_struct *region) {
...
void make_state_FTds6MaintainerCommandsAndMessages__root(
    FTds6MaintainerCommandsAndMessages *self,
  ...
  make_state_FTds6MaintainerCommandsAndMessages__root__ReceivingCommandsAndReportMessages(
      self, x);
  return;
}
...
void transition_from_FTds6MaintainerCommandsAndMessages__root(
    FTds6MaintainerCommandsAndMessages *self,
    FTds6MaintainerCommandsAndMessages__root__state_struct *x) {
  switch (x->state) {
  case FTds6MaintainerCommandsAndMessages__root__ReceivingCommandsAndReportMessages:
    transition_from_FTds6MaintainerCommandsAndMessages__root__ReceivingCommandsAndReportMessages(
        self, x);
    break;
  }
}
...
static void evaluateChangeEvents(FTds6MaintainerCommandsAndMessages *self) {
  self->Change1096.IsTriggered = self->T34inDrfc.IsTriggered;
  self->Change1097.IsTriggered = self->T35inFc.IsTriggered;
  self->Change1098.IsTriggered = self->T35inFc.IsTriggered;
  self->Change1158.IsTriggered = self->T41inVisualSweepingConfirmed.IsTriggered;
}
