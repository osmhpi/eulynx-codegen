...
void make_state_SSciLsCommandExpl__root__SendingCommands(
    SSciLsCommandExpl *self, SSciLsCommandExpl__root__state_struct *region) {
  LOG("[SSciLsCommandExpl] Entering state SendingCommands in region root");
  region->state = SSciLsCommandExpl__root__SendingCommands;
};

void make_state_SSciLsCommandExpl__root(
    SSciLsCommandExpl *self, SSciLsCommandExpl__root__state_struct *x) {
...
void transition_from_SSciLsCommandExpl__root__SendingCommands(
    SSciLsCommandExpl *self, SSciLsCommandExpl__root__state_struct *x) {
  ...
  if (self->Change1010.IsTriggered) {
    if ((self->D23inSignalAspect.Value ==
  ...
  if (self->Change1021.IsTriggered) {
  ...
  if (self->Change1023.IsTriggered) {
    if ((self->D25inLuminosity.Value ==
...
void transition_from_SSciLsCommandExpl__root(
    SSciLsCommandExpl *self, SSciLsCommandExpl__root__state_struct *x) {
  switch (x->state) {
...
static void evaluateChangeEvents(SSciLsCommandExpl *self) {
  self->Change1015.IsTriggered = self->T23inSignalAspect.IsTriggered;
  self->Change1010.IsTriggered = self->T23inSignalAspect.IsTriggered;
  self->Change1014.IsTriggered = self->T23inSignalAspect.IsTriggered;
  self->Change1021.IsTriggered = self->T25inLuminosity.IsTriggered;
  self->Change1023.IsTriggered = self->T25inLuminosity.IsTriggered;
}
